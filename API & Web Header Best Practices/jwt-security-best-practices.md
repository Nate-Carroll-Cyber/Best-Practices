...
4. **Validate `iss`, `aud`, `exp`, `nbf`, and `iat`.**
   The common mistake is verifying the signature but not checking whether the token was meant for your API. RFC 7519 defines `aud` as the intended recipient and says a recipient not listed in `aud` must reject the token; it also defines `exp`, `nbf`, `iat`, and `jti`. ([IETF Datatracker][2])
...
8. **Bind tokens to context where practical.**
   For browser sessions, consider a hardened cookie-bound fingerprint or sender-constrained token pattern so a stolen bearer token is less useful. OWASP describes using a random hardened cookie value and storing only its SHA-256 hash in the JWT to detect replay. ([OWASP Cheat Sheet Series][3])
...
typ: explicit token type where possible
kid: resolved only from trusted JWKS for that issuer
claims: minimal, non-sensitive
transport: HTTPS only
```

So yes: **sign with RS256**, but do not stop there. The secure design is **fixed algorithm + strict claim validation + short TTL + revocation plan + safe storage + key rotation + token-type separation**.
